...
public class StaffPanel extends MainPanel {
	...
	public void getAllStaffs() {
		ArrayList<StaffDTO> list;
		try {
			list = serviceHelper.getAllStaffs();
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (cmd.equals(ADD_EVENT)) {
				...
				if (dialog.dialogResult != null) {
					StaffDTO dto = dialog.dialogResult.entity;
				...
				StaffDTO dto = tableModel.getListData().get(selectedRow);
				StaffAddDialog dialog = new StaffAddDialog();
				dialog.setModal(true);
				dialog.setSize(650, 350);
				...
				StaffDTO dto = tableModel.getListData().get(selectedRow);
				try {
					serviceHelper.deleteStaffDTO(dto);
					tableModel.getListData().remove(dto);
					tableModel.fireTableDataChanged();
				} catch (Exception ee) {
					ee.printStackTrace();
					showError("Cannot delete staff!");
